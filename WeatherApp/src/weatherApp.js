...
async function fetchWeatherData(city) {
  ...
  try {
    ...
    // Controllo del formato della risposta
    ...
    // Elaborazione del forecast per i prossimi 3 giorni
    const forecast = [];
    const currentDate = new Date(currentData.dt_txt).toISOString().slice(0, 10);

    // Utilizzo di un oggetto per raggruppare i dati per data
    const dailyData = {};
    ...
    // Per ogni giorno, utilizzo del dato più vicino a mezzogiorno (12:00:00)
    ...
    for (let i = 0; i < Math.min(3, dates.length); i++) {
      ...
      // Elemento che ha orario più vicino a mezzogiorno
      const targetHour = 12;
...
/**
 * Aggiornamento del DOM per mostrare i dati meteo all'utente.
 * Visualizzazione della sezione per il meteo corrente e per le previsioni future.
 */
function displayWeatherData(data) {
  ...
  // Creazione della sezione per il meteo corrente
  const currentHTML = `
  ...
  // Creazione della sezione per il forecast
...
// Aggiornamento del DOM per mostrare messaggi di stato o errori all'utente.
